<html lang='en'>
	<head>
		<meta charset='utf-8'>
		<meta name='robots' content="noindex">
		<meta name='viewport' content='width=device-width, initial-scale=1'>
        <link rel='stylesheet' href='/assets/bootstrap.css'>
        <link rel='stylesheet' href="/assets/homepage/index.css">
        <script src='/assets/jquery.js'></script>
        <script src='/assets/bootstrap.js'></script>
        <script src='/assets/homepage/homepage.js'></script>
    </head>
<body>

<div class="row text-center black-bg">
    <div class="col-12">
        <div id="navtop">
            <div class="row">
                <div class="col-5">
                    <a href="index.php">Home</a>
                    <a href="#" id="consoles">Games &#8615</a>
                    <a href="#">Deals</a>
                </div>
                <div class="col-2">
                    <a href="#cons-list" class="home-header">PSDB</a>
                </div>
                <div class="col-5">
                    <a href="#">Wish List</a>
                    <a href="#">Placeholder</a>
                    <a href="#">Log In</a>
                </div>
            </div>
        </div>
    </div>
</div>

<div id="cons-list" class="text-center hide black-bg">
    <ul>
        <li>PS4</li>
        <li>PS3</li>
        <li>PS2</li>
        <li>PS1</li>
    </ul>
</div>

<section class="main-body-content margin-top-150">
    <div class="row">
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/ghosts-of-tsu.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Ghosts Of Tsushima</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>              
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/thps.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Tony Hawk's™ Pro Skater™ 1 + 2</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span> 
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/marvel.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Marvel's Avengers</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/cyber.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Cyberpunk 2077</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row margin-top-50">
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/ratchet.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Ratchet and Clank™: Tools of Destruction</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/last.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">The Last of Us™ Remastered</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/until.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Until Dawn™</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
        <div class="col-3">
            <div class="game-card">
                <img class="game-image" src="/assets/images/black.jpeg">
                <div class="row">
                    <div class="col-12">
                        <h3 class="game-titles">Call of Duty®: Black Ops™ Gold Edition</h3>
                        <h3 class="game-attr">Bundle</h3>
                        <h3 class="game-cons">PS4</h3>
                        <h2 class="game-price">£54.99</h2>
                        <span class="favBtn"><span></span></span>  
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>




<footer class="fixed-bottom text-center black-bg">
    <div class="row">
        <div class="col-12">
        </div>
    </div>
</footer>
</body>
